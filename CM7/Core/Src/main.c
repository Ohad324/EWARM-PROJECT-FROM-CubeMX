...
static void MX_DFSDM1_Init(void)
{
  ...
  /* ── Path C-PC2: Channel 0 = clock master, drives 2 MHz CKOUT on PC2/AF6 ───
   * pclk2 = 100 MHz, Divider = 50 → CKOUT = 2.000 MHz on PC2.
   ...
   * PE2 net to SB45 → mic CLK pad. DFSDM samples PC1/DATIN1 against the same
   * internal CKOUT it drives out → AN5027 §2.4.2 satisfied by physics. */
  hdfsdm1_channel0.Instance                    = DFSDM1_Channel0;
  hdfsdm1_channel0.Init.OutputClock.Activation = ENABLE;
  hdfsdm1_channel0.Init.OutputClock.Selection  = DFSDM_CHANNEL_OUTPUT_CLOCK_SYSTEM; /* CKOUTSRC=0: kernel = pclk2 */
  hdfsdm1_channel0.Init.OutputClock.Divider    = 50u;  /* 100 MHz / 50 = 2.000 MHz */
  /* CHEN stays disabled — CH0 is clock-master only, no data path */
   ...
   * SPICKSEL=01: clock taken from internal CKOUT generated by Channel 0
   *   (Path C-PC2). DFSDM samples DATIN1 against the same CKOUT edges that
   *   PC2/AF6 drives out the wire to the mic → mic data eye and DFSDM sample
   *   edges share one silicon node → AN5027 §2.4.2 satisfied by physics.
   *   Target CHCFGR1 = 0x00000085 (CHEN|SPICKSEL=01|SITP=01).
  ...
  hdfsdm1_channel1.Init.SerialInterface.SpiClock   = DFSDM_CHANNEL_SPI_CLOCK_INTERNAL;  /* SPICKSEL=01: internal CKOUT from CH0. CHCFGR1 target = 0x85 (Path C-PC2) */
  ...
  g_dbg.spicksel    = (g_dbg.ch0_cfg1 >> 2u) & 0x3u;           /* expect 1 = internal CKOUT (Path C-PC2) */
